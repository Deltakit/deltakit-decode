...
class StimNoise(
    ...
    def __init__(
        self,
        before_gate_noise_profile: sp.NoiseProfile
        | Iterable[sp.NoiseProfile]
        | None = None,
        after_gate_noise_profile: sp.NoiseProfile
        | Iterable[sp.NoiseProfile]
        | None = None,
        gate_replacement_policy: sp.GateReplacementPolicy | None = None,
        batch_size=1024,
    ):
        self._batch_size = batch_size
        self._before_gate_noise_profile: sp.NoiseProfile | Iterable[sp.NoiseProfile] = (
            before_gate_noise_profile
            if before_gate_noise_profile is not None
            else give_empty
        )
        self._after_gate_noise_profile: sp.NoiseProfile | Iterable[sp.NoiseProfile] = (
            after_gate_noise_profile
            if after_gate_noise_profile is not None
            else give_empty
        )
        self._gate_replacement_policy: sp.GateReplacementPolicy = (
            gate_replacement_policy if gate_replacement_policy is not None else {}
        )
        self._batch_size = batch_size
...
class OptionedStim(StimNoise):
    ...
    def __init__(
        self,
        after_clifford_depolarisation: float = 0.0,
    ...
    def field_values(self) -> dict[str, Any]:
        ...
        base_dict["after_clifford_depolarisation"] = self._after_clifford_depolarisation
        base_dict["before_measure_flip_probability"] = (
            self._before_measure_flip_probability
        )
        base_dict["after_reset_flip_probability"] = self._after_reset_flip_probability
        return base_dict
    ...
    def __init__(self, p_physical: float):
        ...
        after_gate_noise += [
            sp.noise_profile_with_inverted_noise(
                target_gate_t, target_noise_generator=depolarise2_generator
            )
        ...
        after_gate_noise.append(
            sp.noise_profile_with_inverted_noise(
                None, inverse_noise_generator=depolarise1_generator
            )
        )
    ...
    for sample in detector_batch:
        if num_observables:
            detectors, observables = (
                sample[:-num_observables],
                sample[-num_observables:],
            )
        else:
            detectors, observables = sample, []
        yield (
            OrderedSyndrome.from_bitstring(detectors),
