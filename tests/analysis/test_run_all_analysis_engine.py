...
def mock_all_fails_run_batch_shots(
    batch_size: int, decoder_manager: DecoderManager
) -> None:
...
def mock_half_fails_run_batch_shots(
    batch_size: int, decoder_manager: DecoderManager
) -> None:
...
mock_half_fails_run_batch_shots_typed: MockRunBatchShots = (
    mock_half_fails_run_batch_shots
)
...
def all_fail_decoder_managers(mocker, num_decoder_managers) -> list[DecoderManager]:
    return mock_decoder_managers(
        mocker, num_decoder_managers, mock_all_fails_run_batch_shots_typed
    )
...
def half_fail_decoder_managers(mocker, num_decoder_managers) -> list[DecoderManager]:
    return mock_decoder_managers(
        mocker, num_decoder_managers, mock_half_fails_run_batch_shots_typed
    )


def mock_decoder_managers(
    mocker,
    ...
    for _ in range(num_decoder_managers):
        get_reporter_results: MagicMock = mocker.MagicMock(
            return_value={"test_field_name": "test_field_value"}
        )
        ...
        type(
            mock_decoder_manager
        ).empirical_decoding_error_distribution = mock_decoding_distribution
...
def test_error_raised_for_invalid_data_directory(all_fail_decoder_managers, tmp_path):
...
def test_run_outputs_to_file_when_data_directory_is_given(
    all_fail_decoder_managers, tmp_path
):
...
def test_loop_until_failures_runs_correct_number_of_shots(
    all_fail_decoder_managers, random_generator
):
...
def test_max_shots_param_runs_correct_number_of_shots(
    all_fail_decoder_managers, random_generator
):
    shot_num = random_generator.integers(10, 100)
...
def test_loop_until_observable_rse_below_threshold_runs_correct_number_of_shots(
    all_fail_decoder_managers, random_generator
):
...
def test_loop_until_observable_rse_below_threshold_stops_below_threshold(
    half_fail_decoder_managers,
):
