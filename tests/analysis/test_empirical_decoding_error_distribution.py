...
class TestEmpiricalDecodingErrorDistribution:
    ...
    def _distribution_data_is_equal(
        self,
    ...
    def test_from_dict_then_to_dict_is_identity(self, num_logicals: int):
        ...
        for _ in range(10):
            empirical_decoding_error_distribution.add_event(
                self._generate_random_bool_tuple(event_length), random.randint(1, 99)
            )
        assert (
    ...
    @pytest.mark.parametrize("bad_tuple", [(True,), (True, False, False, True)])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({(True, False, False): 5, (True, True, True): 6, (False, False, True): 9}),
            ({(False,): 0}),
        ],
    )
    def test_from_dict_gives_expected_frequency_from_bool_tuple(
        self, error_distribution_dict: dict[tuple[bool, ...], int]
    ):
    ...
    def test_addition_is_commutative(self, num_logicals: int):
        ...
        for _ in range(10):
            distr1.add_event(
                self._generate_random_bool_tuple(num_logicals), random.randint(1, 99)
            )
            distr2.add_event(
                self._generate_random_bool_tuple(num_logicals), random.randint(1, 99)
            )
    ...
    def test_addition_is_associative(self, num_logicals: int):
        ...
        self._distribution_data_is_equal(
            distr1 + (distr2 + distr3), (distr1 + distr2) + distr3
        )
    ...
    def test_add_empty_distribution_is_identity(self, num_logicals: int):
        ...
        self._distribution_data_is_equal(
            distr1 + EmpiricalDecodingErrorDistribution(num_logicals), distr1
        )

    def test_batch_record_adds_correct_errors(self):
        target = np.array([[1, 1, 1], [1, 0, 1], [0, 1, 1]])

        corrections = np.array([[1, 1, 0], [0, 1, 1], [1, 0, 1]])
    ...
    def test_batch_record_equivalent_to_singly_record_error(self, event_length: int):
        ...
        distr2.batch_record_errors(
            np.array(corrections).astype(np.uint8), np.array(targets).astype(np.uint8)
        )
    ...
    def test_number_of_failures_on_a_given_logical_can_be_computed_from_dict(
        ...
        for i, expected_error_count in enumerate(expected_error_counts):
            assert expected_error_count == distr.get_num_errors_on_logical(logical=i)
